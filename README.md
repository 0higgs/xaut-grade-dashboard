...
面向西安理工大学本科教务系统的非官方成绩与课表看板。用户在同一个网页中完成验证码登录后，系统自动查询全部可用学期成绩和当前理论课表，并计算加权平均分、加权平均绩点、学分和学期趋势。
...
- 加权平均分、加权平均绩点和已获学分
- 学期分析、课程明细、CSV 导入导出
- 自动读取“培养管理 → 我的课表 → 学期理论课表”，支持切换学期
- 直接读取教务系统首页官方本周课表，展示当天课表、当前课程和下一节课
- 账号和密码不写入数据库或浏览器存储
...
- `/api/schedule`：获取可用学期和所选学期理论课表
- `/api/week-schedule`：读取教务系统首页的当前周次、日期和本周课程
- `/api/logout`：立即清除加密会话

成绩数据解析后仅保存在用户当前浏览器的 `localStorage`，课表仅在当前页面会话中展示。Pages Function 不使用数据库，不保存账号、密码、成绩或课表。
